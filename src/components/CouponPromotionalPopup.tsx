...
import { Star, Clock, Percent, GraduationCap } from "lucide-react";
...
const CouponPromotionalPopup: React.FC<CouponPromotionalPopupProps> = ({
  ...
  const popup1Content = {
    ...
    discountText: "25% OFF",
    description:
      "Flat 25% off from the total session when coupon code 'CUEPHORIA25' is used in the checkout.",
  ...
  const popup2Content = {
    ...
    discountText: "50% OFF",
    description:
      "Special offer for NIT Trichy students. Use code 'NIT50' to get 50% off from the bill.",
    couponCode: "NIT50",
  ...
  const currentContent = showCount === 2 ? popup2Content : popup1Content;
      ...
      <DialogContent className="bg-gradient-to-br from-cuephoria-dark via-cuephoria-darkpurple to-cuephoria-dark border-2 border-yellow-400/50 text-white max-w-md animate-scale-in shadow-2xl shadow-yellow-400/20">
        <DialogHeader className="text-center space-y-4 pt-2">
          <div className="flex justify-center">
            <Badge
              className={`bg-gradient-to-r ${currentContent.bgColor} text-black font-bold text-lg px-4 py-2 animate-pulse`}
            >
        ...
        <div className="space-y-6 text-center px-2">
          <div className="space-y-2">
            <p className="text-lg font-semibold text-yellow-200">{currentContent.description}</p>

            <div className="mt-4 p-4 bg-gradient-to-r from-purple-900/40 to-pink-900/40 rounded-lg border border-purple-400/30">
              <p className="text-sm text-purple-200 font-medium mb-2">💫 Coupon Code:</p>
